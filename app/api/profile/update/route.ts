...
import { verifyAuth } from '@/lib/utils/server-auth'; // Your auth util
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Validate required fields (looser for updates)
    ...
    // Fetch existing user doc (for timestamps)
    ...
    const userData = {
      ...
      setupComplete: true, // Ensure it's marked complete
      createdAt,
    ...
    // Update patient document (full details) – always merge
    ...
    await patientRef.set({
      ...
      // Preserve other sections (emergency, medical) if they exist
      ...(existingUserData.emergencyContact ? { emergencyContact: existingUserData.emergencyContact } : { emergencyContact: {} }),
